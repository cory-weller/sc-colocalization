...
run_susie <- function(eQTL.DT, diagnosis.DT) {
    ...
    res <- coloc.susie(diagnosis, eQTL)
    res <- res$results
    setDT(res)
    return(res)
}
...
N <- 1
TissueN <- 1
diagnosis <- COLOC_CLUSTERS[N,]$GWAS
CHROM <- COLOC_CLUSTERS[N,]$CHR
start <- COLOC_CLUSTERS[N,]$extended_bp_start
stop <- COLOC_CLUSTERS[N,]$extended_bp_stop
tissue <- TISSUES[TissueN]
...
dt.all <- foreach(diagnosis=COLOC_CLUSTERS$GWAS,
        ...
        .combine='rbind', .errorhandling='remove') %do% {
            ...
            o <- foreach(tissue=TISSUES, .combine='rbind', .errorhandling='remove') %do% {
                ...
                coloc.dt <- run_susie(eQTL_gwas, diagnosis_gwas)
